...
def plot_covariance_analysis(real_csi, gen_csi, cov_matrix, target_t, save_path):
    """
    绘制真实CSI、生成CSI、1024x1024协方差矩阵以及32x32方差热力图
    """
    # 1. 数据预处理
    real = real_csi.squeeze() # [32, 32]
    gen = gen_csi.squeeze()   # [32, 32]
    
    # 提取完整协方差矩阵 (转为 numpy)
    cov_np = (cov_matrix[TARGET]).cpu().numpy() # [1024, 1024]
    
    # 提取对角线元素 (即每个像素的方差)，并重塑为物理场维度
    variance_1d = np.diag(cov_np)
    variance_2d = variance_1d.reshape(32, 32) # [32, 32]
...
def main():
    ...
    SAVE_DIR = "/home/gshang/.AAAHAR/Diffusion/FHcov_analysis_results"
    DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    os.makedirs(SAVE_DIR, exist_ok=True)
    
    # 设定你想观察的特定时间步 (例如: t=50 表示处于中等噪声水平时的不确定度)
    TARGET_T = [TARGET]  # 你可以修改为任何在 diffusion.timesteps 范围内的值
    
    print(f"[*] 运行设备: {DEVICE}")
    print(f"[*] 目标观测时间步: t={TARGET_T}")
    ...
    D_init, Gamma = compute_DCT_prior_variance(train_loader,device=DEVICE)
    torch.save((D_init, Gamma), "/home/gshang/.AAAHAR/Diffusion/pretrained_DCT_prior.pth")
  
    D_init, Gamma = torch.load("/home/gshang/.AAAHAR/Diffusion/pretrained_DCT_prior.pth")
    # ---------- 构建掩码 ----------
    mask = torch.zeros((1, 1, 32, 32), dtype=torch.float32).to(DEVICE)
    mask[0, 0, 0, 0:32:5] = 1.0
    ...
    diffusion = CSIDiffusion(unet, timesteps=10).to(DEVICE)
    diffusion.eval()
    ...
    with torch.no_grad():
        for idx, (inputs, labels) in enumerate(test_loader):
            ...
            print(f"     当执行到 t={TARGET_T} 时可能会卡顿数秒以计算 2800x2800 Jacobian，请稍候...")
            ...
            T1 = time.time()
            with torch.no_grad():
                (gen_csi, cov_matrix) = diffusion.sample_with_FHcovariance(
                mask=mask, 
                cond=cond, 
                target_cov_t_list=TARGET_T,
            ...
            print("cov_matrix 包含的键：", list(cov_matrix.keys()))
            ...
            save_path = os.path.join(SAVE_DIR, f"cov_analysis_sample_{idx+1}_t{TARGET_T}.png")
