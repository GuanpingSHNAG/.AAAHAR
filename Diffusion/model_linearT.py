...
class CSIDiffusion(nn.Module):
    def __init__(self, model, timesteps=20):
        super().__init__()
        self.model = model
        self.timesteps = timesteps
        
        # 线性 Noise Schedule
        beta = torch.linspace(1e-4, 30, timesteps)
    ...
    def sample_with_FHcovariance(self, mask, cond, target_cov_t_list=None, D_init=None, Gamma=None, return_all=False):
        ...
        for t in reversed(range(self.timesteps)):
            ...
            # 等效噪声映射：在新排期下，sigma_t 实际上等于 torch.sqrt(self.beta[t])
...
# ==========================================================
            ...
            if sigma_prev is not None:
               # 针对 T=20，物理噪声的绝对跨度是从 0.01 到 3.16。
                # 0.05 对应倒数极早的收尾阶段，放开此窗口能最大程度利用曲率。
                is_active_window = 0.5 <= sigma_t <= 3.0
                #is_active_window =1
    ...
    @torch.no_grad() # Free Hunch 纯前向，完全不需要开启梯度！极其快速！
    def sample_with_FH_mean_covariance(self, mask, cond, target_cov_t_list, D_init=None, Gamma=None, return_all=False):
        """
        使用 Free Hunch 方法在线迭代计算，并直接返回指定时间步(target_cov_t_list)的平均协方差矩阵。
        """
        if not target_cov_t_list:
            raise ValueError("target_cov_t_list 不能为空！")
        ...
        # 1. 容错初始化 D_init 和 Gamma (如果你外部没传，默认空间域单位矩阵)
        if D_init is None:
            D_init = torch.ones(N, device=device) 
        if Gamma is None:
            Gamma = torch.eye(N, device=device)
        ...
        x_prev, mu_prev, sigma_prev = None, None, None
        ...
        for t in reversed(range(self.timesteps)):
            ...
            # ==========================================================
            if sigma_prev is not None:
                # 遵循你的 is_active_window 设定
                is_active_window = 0.5 <= sigma_t <= 3.0
                if is_active_window:
                    mu_prev_aligned = fh_manager.analytical_score_transport(
                        x_spatial=x_prev, mu_curr_spatial=mu_prev, 
                        sigma_curr=sigma_prev, sigma_next=sigma_t
                    )
                else:
                    mu_prev_aligned = mu_prev 
            ...
            x_prev, mu_prev, sigma_prev = x_t.clone(), mu_t.clone(), sigma_t
            
            # ==========================================================
            # --- 拦截点：均值累加 (极速获取并累加) ---
            # ==========================================================
            if t in target_cov_t_list:
                curr_cov = fh_manager.get_full_matrix()
                if cov_sum is None:
                    cov_sum = curr_cov.clone()
                else:
                    cov_sum += curr_cov
                cov_count += 1
        ...
        mean_cov = (cov_sum / cov_count) if cov_count > 0 else None
    ...
    @torch.no_grad()
    def sample_with_autograd_mean_covariance(self, mask, cond, target_cov_t_list, return_all=False):
        """
        基于原生的 Autograd (Jacobian) 在线迭代计算，并返回指定时间步的精确平均协方差矩阵。
        注意：该方法计算极其精确，但耗时较长，且要求 batch_size 必须为 1！
        """
        if not target_cov_t_list:
            raise ValueError("target_cov_t_list 不能为空！")
            
        B = cond.shape[0]
        assert B == 1, "Autograd 计算 Jacobian 矩阵极其吃显存，强制要求 Batch Size = 1！"
        ...
        for t in reversed(range(self.timesteps)):
            # ==========================================================
            # --- 拦截点：利用 Autograd 提取真实 Jacobian 协方差 ---
            # ==========================================================
            if t in target_cov_t_list:
                # 必须临时开启计算图以获取 Jacobian
                with torch.enable_grad():
                    x_t_req_grad = x_t.clone().detach().requires_grad_(True)
                    cov_t = self.get_covariance_matrix(x_t_req_grad, t, mask, cond)
                
                if cov_sum is None:
                    cov_sum = cov_t.detach().clone()
                else:
                    cov_sum += cov_t.detach()
                cov_count += 1
