...
from PowerAlloc import Allocator
...
def main():
    ...
    for epoch in range(EPOCHS):
        ...
        if avg_val_acc > best_val_acc:
            ...
            save_path = os.path.join(SAVE_DIR, f"best_model-SNR_pilots={NUM_DYNAMIC_PILOTS}.pth")
            torch.save({
                'epoch': epoch + 1,
    ...
    json_path = os.path.join(SAVE_DIR, f'history_MLP_pilots={NUM_DYNAMIC_PILOTS}.json')
    with open(json_path, 'w', encoding='utf-8') as f:
        json.dump(history_data, f, indent=4, ensure_ascii=False)
    print(f"[*] 🟢 MLP 历史数据已保存至 JSON: {json_path}")
    ...
    plt.legend()
    plt.grid(True)
    curve_path = os.path.join(SAVE_DIR, f'归一化功率training_curves_varSNR_Pilots={NUM_DYNAMIC_PILOTS}.png')
