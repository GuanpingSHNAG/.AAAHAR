...
def main():
    ...
    for NUM_DYNAMIC_PILOTS in NP_LIST:
        print(f"\n{'='*60}")
        print(f"🚀 开始端到端联合训练 (UniPow Baseline) | Np = {NUM_DYNAMIC_PILOTS}")
        print(f"{'='*60}")
        ...
        optimizer = optim.AdamW([
            {'params': allocator.parameters(), 'lr': 0},  # 保持原代码的 lr=0 逻辑
    ...
    print("\n\n" + "="*70)
    print("🎯 所有 Np 的最优验证结果汇总 (0-30dB 各自 + 平均的 Acc & F1) [UniPow Baseline]")
    print("="*70)
