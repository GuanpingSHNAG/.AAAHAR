...
def main():
    ...
    sparse_mlp.load_state_dict(torch.load("/home/gshang/.AAAHAR/Csem/稀疏随机MLP/Full.pth", map_location=DEVICE))

    # 【重点修改】将 allocator 的参数也加入优化器
    optimizer = optim.AdamW([
        {'params': allocator.parameters(), 'lr': 5e-4}, # 可以给功率分配网络单独设一个学习率
        {'params': sparse_mlp.parameters(), 'lr': 5e-4}
    ], weight_decay=1e-4)
    
    
    #    直接初始化，无需列表包裹SequentialLR
    ...
    for epoch in range(EPOCHS):
        ...
        if avg_val_acc > best_val_acc:
            ...
            save_path = os.path.join(SAVE_DIR, f"Uni_pow_best_model-SNR_pilots={NUM_DYNAMIC_PILOTS}.pth")
            torch.save({
                'epoch': epoch + 1,
   ...
   # ================= [NEW] 将训练历史保存为 JSON 文件 =================
    import json
    history_data = {
        "method": "UniPow Baseline",
        "num_dynamic_pilots": NUM_DYNAMIC_PILOTS,
        "epochs": list(range(1, EPOCHS + 1)),
        "train_accs": train_accs, 
        "val_accs": val_accs      
    }
    json_path = os.path.join(SAVE_DIR, f'history_UniPow_pilots={NUM_DYNAMIC_PILOTS}.json')
    with open(json_path, 'w', encoding='utf-8') as f:
        json.dump(history_data, f, indent=4, ensure_ascii=False)
    print(f"[*] 🟢 UniPow 历史数据已保存至 JSON: {json_path}")
    # ====================================================================
    ...
    plt.xlabel('Epoch')
    plt.ylabel('Accuracy (%)')
    plt.title('Training and Validation Accuracy Curves - UniPow')
    plt.legend()
    plt.grid(True)
    curve_path = os.path.join(SAVE_DIR, f'Uni_pow_training_curves_varSNR_Pilots={NUM_DYNAMIC_PILOTS}.png')
