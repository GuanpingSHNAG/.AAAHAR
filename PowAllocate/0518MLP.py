...
from PowerAlloc import Allocator  # 这里对应你原脚本的导入
...
def main():
    ...
    for NUM_DYNAMIC_PILOTS in NP_LIST:
        print(f"\n{'='*60}")
        print(f"🚀 开始端到端联合训练 (MLP Baseline) | Np = {NUM_DYNAMIC_PILOTS}")
        print(f"{'='*60}")
    ...
    print("\n\n" + "="*70)
    print("🎯 所有 Np 的最优验证结果汇总 (0-30dB 各自 + 平均的 Acc & F1) [MLP Baseline]")
    print("="*70)
