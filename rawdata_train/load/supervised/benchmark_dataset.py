...
# ================= 原有的 500x56 标准化逻辑 =================
            # Standardize the size to (1, 500, 56)
            batch_size, time_index, feature_size = csi_data.shape
            target_time_index, target_feature_size = 500, 56
            ...
            # ================= 新增：时间维度降维 (500 -> 50) =================
            # 策略 A：平均池化 (Average Pooling) - 推荐！具有抗噪和平滑效果
            # 将 500 拆分成 50 个时间窗口，每个窗口包含 10 个采样点，然后求平均
            
            
            # 策略 B：直接降采样 (Downsampling) - 如果你想保留瞬态特征，用这行注释替换上面那行
            standardized_data = standardized_data[:,::10, :]
            #standardized_data = standardized_data.view(batch_size, 1, 28, 2).mean(dim=-1)
            """================ 新增：添加高斯噪声 (可选) =================
            这段代码会在标准化后的数据上添加高斯噪声，以模拟更真实的环境。。
            
            snr_db = 10  # 修改此值调节信噪比
            standardized_data += torch.randn_like(standardized_data) * torch.sqrt(torch.mean(standardized_data**2) / (10**(snr_db/10)))
            """
            ...
            '''================ 新增：random稀疏化 (可选) =================
            这段代码会随机将部分数据置零，以模拟稀疏导频情况。你可以调整 sparsity 参数来控制稀疏程度。
            '''
            # sparsity = 0.1  # 10% 的数据保留，90% 的数据置零
            # mask = (torch.rand_like(csi_data) < sparsity).float()
            # csi_data = csi_data * mask

            '''================ 新增：uniform稀疏化 (可选) =================
            这段代码会按照固定的步长保留数据点，其他点置零，以模拟uniform稀疏导频情况。你可以调整 step 参数来控制'''
            # # 设置每维的保留比例（例如行 0.4，列 0.25）
            # ratio_t, ratio_f = 0.2, 0.05

            # # 计算步长（至少为1）
            # step_h = int(round(1 / ratio_t))
            # step_w = int(round(1 / ratio_f))

            # # 生成 mask 并应用
            # mask = torch.zeros_like(csi_data)
            # mask[..., ::step_h, ::step_w] = 1
            # csi_data = csi_data * mask
